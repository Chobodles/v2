...
if (str_starts_with($ref, "BRGY-")) {
    ...
    try {
        ...
        $priceDisplay =
            $rawPrice == 0 ? "Free" : "₱" . number_format((float) $rawPrice, 2);

        $dateReleased = $row["date_released"];
        if (
            $dateReleased === "0000-00-00" ||
            $dateReleased === "0000-00-00 00:00:00"
        ) {
            $dateReleased = null;
        }
    ...
    echo json_encode([
        "success" => false,
        "message" =>
            "Invalid reference number format. Use BRGY-YEAR-XXXX or DOC-YEAR-XXXX.",
    ]);
}
exit();
?>
